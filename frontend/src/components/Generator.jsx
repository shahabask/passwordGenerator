...
const deleteHandler = async (id) => {
  ...
  return (
    <div className='h-5/6 flex flex-col bg-sky-200 justify-between'>
        <div className='h-full flex justify-between'>
            <div className='h-full w-1/2 flex justify-center items-center'>
                <div className='bg-sky-900 w-3/6 p-5 rounded-lg'>
                    <div className='w-full h-8 flex'>
                        <input type="text" value={result} className='h-full p-3 w-5/6 rounded-tl rounded-bl focus:outline-none' readOnly/>
                        <div onClick={handleClipboard} className='bg-sky-200 cursor-pointer flex justify-center items-center p-2 w-1/6 rounded-tr rounded-br'>
                            <BsClipboardFill className='text-xl text-black'/>
                        </div>
                    </div>
                    {
                        result ? (
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className='my-2 w-full'>
                            <button type="submit" className='bg-white hover:bg-quaternaryColor w-full p-2 rounded-lg text-black font-medium'>Generate Password</button>
                        </div>
                    </form>
                </div>
            </div>
            <div className='h-5/6 w-1/2 bg-sky-900 mt-10 mx-5 rounded-lg'>
                ...
                <div className='overflow-y-auto h-5/6'>
                    {
                        collection.length > 0 ? (
                            collection.map((password) => (
                                <div key={password._id} className='bg-white m-3 p-3 rounded flex justify-between'>
                                    ...
                                    <div className='flex'>
                                        {visible === password._id ? (
                                            <LiaEyeSolid
                                                ...
                                                className='text-2xl mx-1 cursor-pointer text-black'
                                            />
                                        ) : (
                                            <LiaEyeSlash
                                        ...
                                        <BsTrashFill onClick={()=>deleteHandler(password._id)} className='text-2xl mx-1 cursor-pointer text-black' />
                                    </div>
                                </div>
                            ))
                        ) : <div className='w-full h-full flex justify-center items-center'><h1 className='text-white font-bold'>No Saved Passwords</h1></div>
                    }
                </div>
            </div>
        </div>
